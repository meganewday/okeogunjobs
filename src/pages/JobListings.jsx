...
const LGAS = [
  ...
  "Iseyin", "Itesiwaju", "Kajola", "Iwajowa", "Ogbomosho North",
];
...
function SkeletonCard() {
  return (
    <div style={{
      ...
      border: "1.5px solid #dcfce7", animation: "pulse 1.4s ease-in-out infinite",
    }}>
...
function JobCard({ job, index }) {
  ...
  const displayLoc = [job.location, job.lga].filter(Boolean).join(", ") || "Oke-Ogun";
  ...
  return (
    <div style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 10px 28px rgba(22,163,74,0.13)"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 2px 10px rgba(22,163,74,0.06)"; }}
    >
      {/* Top row */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 18, fontWeight: 800, color: "#14532d", margin: "0 0 8px", lineHeight: 1.3 }}>
            {job.job_title}
          </h3>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 14, fontSize: 13, color: "#15803d", fontWeight: 500, marginBottom: 14 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <BuildingIcon /> {employer?.organization_name || "—"}
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ background: "#f0fdf4", color: "#166534", borderRadius: 20, padding: "4px 12px", fontSize: 12, fontWeight: 700, border: "1px solid #bbf7d0" }}>
              {typeLabel}
            </span>
        ...
        {waNumber ? (
          <a
            ...
            style={{
              ...
              padding: "12px 22px", borderRadius: 50, textDecoration: "none",
              boxShadow: "0 4px 12px rgba(22,163,74,0.3)", transition: "transform 0.15s, box-shadow 0.15s",
              whiteSpace: "nowrap", flexShrink: 0,
            }}
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 6px 18px rgba(22,163,74,0.4)"; }}
          ...
          <span style={{
            ...
            background: "#f0fdf4", color: "#166534", fontWeight: 600, fontSize: 13,
            padding: "10px 18px", borderRadius: 50, border: "1.5px solid #bbf7d0",
            whiteSpace: "nowrap", flexShrink: 0,
          }}>
            📞 Call to Apply
          </span>
      ...
      {/* Skills chips */}
      {Array.isArray(job.skills) && job.skills.length > 1 && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #f0fdf4", display: "flex", gap: 6, flexWrap: "wrap" }}>
          {job.skills.slice(0, 5).map(s => (
            <span key={s.name} style={{ background: "#f0fdf4", color: "#15803d", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 600 }}>
              {s.name}
            </span>
          ))}
          {job.skills.length > 5 && (
...
export default function JobListings() {
  ...
  // Fetch all approved jobs + all skills for filter dropdown
  useEffect(() => {
      ...
      try {
        const [jobsRes, skillsRes] = await Promise.all([
          supabase
            .from("job_listings")
            .select(`
              id, job_title, job_type, location, lga, labour_type, application_method, skills_required,
  ...
  const filtered = jobs.filter(job => {
    ...
    const matchSearch = !search ||
      ...
      (Array.isArray(job.skills) && job.skills.some(s => s.name.toLowerCase().includes(q)));

    const matchSkill  = !filterSkill  || (Array.isArray(job.skills) && job.skills.some(s => s.id === filterSkill));
    ...
    <div style={{ fontFamily: "'Outfit', 'Segoe UI', sans-serif", background: "#f0fdf4", minHeight: "100vh" }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{
            ...
            marginBottom: 16, border: "1.5px solid #bbf7d0",
          }}>
            📋 Verified Listings
          </div>
          <h1 style={{ fontSize: "clamp(28px, 4vw, 44px)", fontWeight: 900, color: "#14532d", margin: "0 0 10px" }}>
            Browse Job Listings
          </h1>
          ...
          <div style={{ position: "relative", maxWidth: 560 }}>
            ...
            <input
              type="text"
              placeholder="Search by job title, employer, or skill…"
              value={search}
              onChange={e => setSearch(e.target.value)}
              style={{
                width: "100%", padding: "14px 16px 14px 46px",
                border: "1.5px solid #bbf7d0", borderRadius: 50,
                background: "#fff", fontSize: 14, color: "#14532d",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px 64px", display: "flex", gap: 28, alignItems: "flex-start" }}>
        ...
        <aside style={{
          ...
          border: "1.5px solid #dcfce7", position: "sticky", top: 24,
          display: window.innerWidth < 900 ? "none" : "block",
        }}>
          <FilterPanel
            skills={skills}
            filterSkill={filterSkill} setFilterSkill={setFilterSkill}
            filterLGA={filterLGA}     setFilterLGA={setFilterLGA}
            filterType={filterType}   setFilterType={setFilterType}
            filterLabour={filterLabour} setFilterLabour={setFilterLabour}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: activeFilters > 0 ? "#16a34a" : "#fff",
                  color: activeFilters > 0 ? "#fff" : "#16a34a",
                  border: "1.5px solid #16a34a", borderRadius: 50,
          ...
          {activeFilters > 0 && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              ...
              {filterSkill  && <FilterChip label={skills.find(s => s.id === filterSkill)?.name || filterSkill} onRemove={() => setFilterSkill("")} />}
            </div>
          )}
          ...
          {showFilters && (
            <div style={{
              background: "#fff", borderRadius: 20, padding: "24px 20px",
              border: "1.5px solid #dcfce7", marginBottom: 20,
            }}>
              <FilterPanel
                skills={skills}
                filterSkill={filterSkill} setFilterSkill={setFilterSkill}
          ...
          {!loading && !error && filtered.length === 0 && (
            <div style={{
              ...
              <button onClick={clearFilters} style={{
                ...
                padding: "12px 28px", borderRadius: 50, fontSize: 14, fontWeight: 700, cursor: "pointer",
              }}>
                Clear Filters
              </button>
            </div>
...
// ─── Filter Panel ─────────────────────────────────────────────────────────────
function FilterPanel({ skills, filterSkill, setFilterSkill, filterLGA, setFilterLGA, filterType, setFilterType, filterLabour, setFilterLabour, activeFilters, clearFilters }) {
  const selectStyle = {
    width: "100%", padding: "10px 12px",
    border: "1.5px solid #dcfce7", borderRadius: 12,
    background: "#f0fdf4", color: "#14532d", fontSize: 14,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[["", "All Types"], ["skilled", "Skilled"], ["unskilled", "Unskilled"], ["internship", "Internship / IT"]].map(([val, label]) => (
              <label key={val} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 14, color: filterLabour === val ? "#16a34a" : "#166534", fontWeight: filterLabour === val ? 700 : 500 }}>
                <input
                  type="radio" name="labour" value={val}
                  checked={filterLabour === val}
                  onChange={() => setFilterLabour(val)}
  ...
  return (
    <span style={{
      ...
      <button onClick={onRemove} style={{
        ...
        display: "flex", alignItems: "center", color: "#16a34a",
      }}>
        <CloseIcon />
      </button>
    </span>
